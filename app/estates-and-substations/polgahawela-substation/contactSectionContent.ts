...
const polgahawelaContactCard = subStationCards.find(
  (card) => card.name === 'Polgahawela Sub-station',
);

if (!polgahawelaContactCard) {
  throw new Error('Polgahawela Sub-station contact data was not found.');
}

const details: LocationDetail[] = polgahawelaContactCard.contacts.map((contact) => ({
  label: contact.label,
  value: contact.value,
  href: contact.href,
}));

export const polgahawelaContactSectionContent: EstateSubstationContactSectionContent =
  {
    titlePart1: 'Contact',
    titlePart2: 'Information',
    sideLabel: 'Sub-station',
    orientation: 'details-left',
    mapTitle: 'Polgahawela Sub-station map',
    mapSrc:
