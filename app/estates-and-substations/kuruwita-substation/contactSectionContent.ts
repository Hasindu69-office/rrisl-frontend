...
const kuruwitaContactCard = subStationCards.find(
  (card) => card.name === 'Kuruwita Sub-station',
);

if (!kuruwitaContactCard) {
  throw new Error('Kuruwita Sub-station contact data was not found.');
}

const details: LocationDetail[] = kuruwitaContactCard.contacts.map((contact) => ({
  label: contact.label,
  value: contact.value,
  href: contact.href,
}));

export const kuruwitaContactSectionContent: EstateSubstationContactSectionContent =
  {
    titlePart1: 'Contact',
    titlePart2: 'Information',
    sideLabel: 'Sub-station',
    orientation: 'details-left',
    mapTitle: 'Kuruwita Sub-station map',
    mapSrc:
