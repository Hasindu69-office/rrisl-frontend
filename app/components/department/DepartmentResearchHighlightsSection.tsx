  ...
  return (
    ...
    >
      <div className="relative mx-auto w-full max-w-[360px] pt-6">
        {images.length > 1 ? (
          <>
            <div className="absolute right-2 top-0 h-[78%] w-[84%] rounded-[18px] border border-[#2E7D32]/10 bg-white/80 shadow-[0_14px_30px_rgba(17,17,17,0.05)] transition-transform duration-300 group-hover:-translate-y-1" />
            <div className="absolute right-1 top-3 h-[82%] w-[90%] rounded-[18px] border border-[#2E7D32]/10 bg-white/92 shadow-[0_18px_34px_rgba(17,17,17,0.06)] transition-transform duration-300 group-hover:-translate-y-1.5" />
          </>
        ) : null}

        <div className="relative z-[1] overflow-hidden rounded-[18px] border border-[#2E7D32]/10 bg-white shadow-[0_18px_40px_rgba(17,17,17,0.09)]">
          ...
          <div className="absolute inset-x-0 bottom-0 flex items-end justify-between gap-3 px-4 py-3">
            ...
            {extraCount > 0 ? (
              <span className="inline-flex items-center rounded-full bg-white/90 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.08em] text-[#0F3F1D]">
                +{extraCount} more
              </span>
            ) : null}
          </div>
...
function HighlightContent({
  ...
  return (
    <div className="rounded-[10px] bg-[#E2EDE1] px-4 py-5 md:px-7 md:py-6">
      {item.details ? (
          ...
          >
            {item.sections?.map((section) => {
              ...
              return (
                <div
                  key={section.id}
                  className={`rounded-[14px] border px-4 py-4 md:px-5 ${
                    sectionHasImages
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0 pr-2 md:pr-4">
              ...
              <h3
                ...
                className="mt-2 text-[20px] font-semibold leading-tight text-[#10341B] sm:text-[22px] md:text-[26px]"
              >
          ...
          <div className="grid gap-4 lg:gap-5 xl:grid-cols-[minmax(0,1fr)_320px]">
            <div className="rounded-[22px] border border-[#E1EBDD] bg-white p-2.5 shadow-[0_18px_40px_rgba(15,63,29,0.08)] sm:rounded-[24px] sm:p-3 md:rounded-[26px] md:p-4 lg:rounded-[28px] lg:p-5">
              <div className="rounded-[18px] border border-dashed border-[#D7E4D1] bg-[#FAFCF8] p-2.5 sm:rounded-[20px] sm:p-3 md:rounded-[22px] md:p-4">
                <div className="relative mx-auto h-[clamp(280px,42dvh,420px)] w-full overflow-hidden rounded-[16px] bg-white shadow-[0_18px_40px_rgba(15,63,29,0.08)] sm:h-[min(56dvh,520px)] sm:rounded-[18px] lg:h-[min(58dvh,620px)]">
                  <Image
                    src={activeImage.src}
                    alt={activeImage.alt}
                    fill
            ...
            <aside className="rounded-[24px] border border-[#E1EBDD] bg-white p-5 shadow-[0_18px_40px_rgba(15,63,29,0.06)]">
              <p className="text-[12px] font-semibold uppercase tracking-[0.14em] text-[#7A8C82]">
                Gallery
              </p>
              <p className="mt-2 text-[22px] font-semibold text-[#10341B]">
    ...
    <>
      ...
      >
        ...
        <div className={`relative z-10 mx-auto max-w-[1600px] px-4 md:px-6 lg:px-8 ${containerClassName}`}>
          <div className="flex flex-col gap-8 lg:grid lg:grid-cols-[86px_minmax(0,1fr)] lg:gap-8 xl:grid-cols-[96px_minmax(0,1fr)] xl:gap-9">
            <div className="flex items-start lg:items-stretch lg:justify-start">
              <span className="sr-only">{tagText}</span>

              <div className="lg:-ml-6 lg:flex lg:min-h-[620px] lg:items-center xl:-ml-8">
                <div className="whitespace-nowrap text-left lg:rotate-180 lg:[writing-mode:vertical-rl]">
                  <span className="text-[34px] font-bold leading-none text-white md:text-[42px] lg:text-[56px] xl:text-[64px]">
                    {titlePart1}
                    {titlePart2}
                  </span>
                </div>
            ...
            <div className="min-w-0 lg:pt-4 xl:pt-5">
              <div
                className={`relative ${
                  isMobile
                    ? 'space-y-4'
                    : 'location-details-scroll overflow-y-auto pr-2 md:pr-3'
                }`}
                style={
                  sectionHeight
                    ? {
              ...
              >
                <div
                  className={`relative w-full ${
                    isMobile ? 'space-y-4' : 'space-y-6 xl:space-y-7'
                  }`}
                >
                  ...
                  {highlights.map((item, index) => {
                    ...
                    return (
                      <div
                        key={item.id}
                        className="relative z-[1] grid grid-cols-1 gap-3 lg:grid-cols-[64px_minmax(0,1fr)] lg:gap-5 xl:grid-cols-[72px_minmax(0,1fr)]"
                      >
                        <div className="relative hidden lg:flex lg:w-full lg:justify-center">
                          <div className="relative z-[2] mt-[21px] flex h-[40px] w-[40px] items-center justify-center rounded-full border border-[#2E7D32]/15 bg-white text-[18px] font-semibold leading-none text-[#7FCB19] shadow-[0_8px_18px_rgba(18,76,21,0.12)] xl:mt-[22px] xl:h-[42px] xl:w-[42px]">
                            {String(index + 1).padStart(2, '0')}
                          </div>
                        </div>

                        <article className="rounded-[18px] bg-white px-5 py-4 shadow-[0_16px_36px_rgba(17,17,17,0.08)] transition-all duration-300 ease-out hover:translate-x-1 hover:shadow-[0_20px_42px_rgba(17,17,17,0.12)] md:px-6 md:py-4 lg:rounded-[16px] xl:px-7 xl:py-4">
                          <button
                            type="button"
                            ...
                            className="flex w-full items-start gap-3 text-left md:gap-5"
                          >
                            <div className="flex items-center gap-3 md:gap-4 lg:hidden">
                              <div className="flex h-[38px] w-[38px] shrink-0 items-center justify-center rounded-full border border-[#2E7D32]/15 bg-white text-[16px] font-semibold leading-none text-[#7FCB19] shadow-[0_8px_18px_rgba(18,76,21,0.12)]">
                                {String(index + 1).padStart(2, '0')}
                              </div>

                              <div className="h-9 w-px border-l border-dotted border-[#2E7D32]/25" />
                            </div>

                            <div className="flex h-[50px] w-[50px] shrink-0 items-center justify-center rounded-full bg-[#EFF4EC] md:h-[54px] md:w-[54px]">
                              <Image
                                src={TIMELINE_ICON}
                                alt=""
                                width={26}
                                height={26}
                                className="h-6 w-6 object-contain md:h-[26px] md:w-[26px]"
                                aria-hidden="true"
                              />
                            </div>

                            <div className="min-w-0 flex-1 pt-1">
                              <h3 className="text-[14px] font-medium leading-[1.55] text-[#2E7D32] md:text-[15px] lg:text-[15px] xl:text-[16px]">
                                {item.summary}
                              </h3>
                            </div>
                            ...
                            {isExpandable ? (
                              <div
                                className={`shrink-0 pt-1 text-[#6FC109] transition-transform duration-300 ${
