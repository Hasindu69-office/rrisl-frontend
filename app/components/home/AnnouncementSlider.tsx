...
export default function AnnouncementSlider({ announcements }: AnnouncementSliderProps) {
  ...
  // Calculate visible announcements (2 at a time)
  const totalAnnouncements = announcements.length;
  const maxIndex = Math.max(0, totalAnnouncements - 2);
    ...
    <div className="w-full">
      ...
      <div className="relative overflow-hidden">
        <div className="flex gap-4">
          {announcements.slice(currentIndex, currentIndex + 2).map((announcement, relativeIndex) => {
            const absoluteIndex = currentIndex + relativeIndex;
            const isSelected = relativeIndex === 0; // First one is always selected
            const imageUrl = announcement.image
              ? getOptimizedImageUrl(announcement.image, 'medium') || 
                getStrapiImageUrl(announcement.image)
              : null;
            const isLocalhost = imageUrl?.includes('localhost') || false;
            const summaryText = limitWords(extractTextFromSummary(announcement.summary), 10);

            return (
              <div
                key={announcement.id || absoluteIndex}
              ...
              >
                ...
                <div className="relative w-full h-full">
                  ...
                  >
                    <defs>
                      <clipPath id={`announcement-clip-selected-${absoluteIndex}`}>
                      ...
                      <clipPath id={`announcement-clip-unselected-${absoluteIndex}`}>
                  ...
                  <Link
                    ...
                    className="absolute top-0 right-0 z-20 transition-all duration-300 hover:opacity-90"
                    style={{
                      transform: 'translate(2%, -2%)', // Center the circle on the corner
                    }}
                  >
                    <svg
                      width="70"
                    ...
                    >
                      <path
                        d="M34.4333 69.658C53.2098 69.8313 70.1284 52.9737 70.0046 34.2102C69.8807 14.0356 55.3401 2.98057e-05 34.5076 2.98057e-05C14.7651 -0.0247243 -0.171903 15.3723 0.00149431 35.5469C0.174892 52.7014 17.1183 69.4847 34.4333 69.658Z"
                        fill={isSelected ? '#0F3F1D' : 'white'}
                        stroke={!isSelected ? '#2E7D32' : 'none'}
                  ...
                  <div
                    ...
                    style={{
                      clipPath: isSelected 
                        ? `url(#announcement-clip-selected-${absoluteIndex})`
                        : `url(#announcement-clip-unselected-${absoluteIndex})`,
                    }}
                  >
                    <div
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          {formatTitle(announcement.title)}
                        </h3>
                      </div>
